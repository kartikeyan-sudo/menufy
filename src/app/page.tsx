...
import { QrCode, Sparkles, Send, Smartphone, ChevronRight, CheckCircle2, ShieldCheck } from 'lucide-react';
      ...
      <main className="flex-1 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-16 lg:py-24 flex flex-col items-center text-center justify-center">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6 w-full text-left mt-8">
          <div className={`p-6 rounded-2xl border transition-all ${
            isDark ? 'border-slate-800/80 bg-slate-900/40' : 'border-slate-200 bg-white shadow-sm hover:shadow-md'
          }`}>
            ...
            <p className={`text-sm leading-relaxed ${isDark ? 'text-slate-400' : 'text-slate-600'}`}>
              Upload menu photos or PDFs. Gemini AI automatically parses your categories, dish names, descriptions, and prices.
            </p>
          </div>
          ...
          <div className={`p-6 rounded-2xl border transition-all ${
            ...
            <p className={`text-sm leading-relaxed ${isDark ? 'text-slate-400' : 'text-slate-600'}`}>
              Customers scan a table QR code and browse a mobile-first digital menu with zero app download or mandatory login.
            </p>
          </div>
          ...
          <div className={`p-6 rounded-2xl border transition-all ${
            ...
            <p className={`text-sm leading-relaxed ${isDark ? 'text-slate-400' : 'text-slate-600'}`}>
              Orders are sent directly to your restaurant's Telegram chat with table numbers and itemized totals in real-time.
            </p>
          </div>
        </div>
      </main>
      ...
      <footer className={`border-t py-6 text-center text-xs ${isDark ? 'border-slate-800/60 text-slate-500' : 'border-slate-200 text-slate-600 bg-white'}`}>
        <div className="max-w-7xl mx-auto px-4 flex flex-col sm:flex-row items-center justify-between gap-4">
          ...
          <div className="flex items-center gap-2">
            <ShieldCheck className="w-4 h-4 text-emerald-500" /> Multi-tenant Security & Server-side Price Validation Enabled
          </div>
        </div>
      </footer>
    </div>
